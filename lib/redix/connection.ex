defmodule Redix.Connection do
  ...
  def disconnect({:error, %ConnectionError{} = error} = _error, state) do
    ...
    :ok = Redix.Connection.Adapter.close(state.socket)
  ...
  def handle_call({:commands, commands, request_id}, from, state) do
    ...
    case Redix.Connection.Adapter.send(state.socket, data) do
      :ok ->
        {:noreply, state}
  ...
  defp start_receiver_and_hand_socket(socket, shared_state) do
    ...
    with :ok <- Redix.Connection.Adapter.controlling_process(socket, receiver),
         :ok <- Redix.Connection.Adapter.setopts(socket, active: :once),
         do: {:ok, receiver}
  end
