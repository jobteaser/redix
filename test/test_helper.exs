...
case Redix.Connection.Adapter.connect(String.to_charlist(host), port, []) do
  {:ok, socket} ->
    Redix.Connection.Adapter.close(socket)

  {:error, reason} ->
    Mix.raise("Cannot connect to Redis (http://#{host}:#{port}): #{Redix.Connection.Adapter.format_error(reason)}")
end
